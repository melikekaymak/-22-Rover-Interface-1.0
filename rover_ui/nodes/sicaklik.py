...
class Sicaklik(QFrame):
    ...
    def __init__(self,widget):
        ...
        # else:
        #     self.data = 0
        #rospy.Subscriber('/sensor_Sicaklik',Float64,self.sicaklik_cb)
        ...
        self.timer.timeout.connect(self.sicaklik_cb)
        self.timer.start()
        # Rate.sleep()

    def sicaklik_cb(self):
        if self.data is not None:
            self.y.append(self.data)
            self.x.append(self.counter)
            self.sum=self.sum+self.data
            ave=self.sum/(self.counter+1)+.1
            self.temp_ave.append(ave)
            self.label.setText("Temp:{}".format(self.data))
